			...
			case 'm':
				switch(buf[4]) {
					case 'K':
						fgets(buf, sizeof(buf), file);
						sscanf(buf, "%s %s", buf, buf);
						material = &model->materials[nummaterials];
						material->tex_file= strdup(buf);
						Image* image = loadBMP(material->tex_file);
						material->tex_map = loadTexture(image);
						delete image;
				}
				break;
			case 'd':
...
/* glmDble(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D,tex_road);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
raw: Renders the model to the current OpenGL context using the
 * mode specified.
 *
 * model - initialized GLMmodel structure
...
//			printf("material ==  %f\n", material->diffuse[3]);
		...
		if(GLM_TEXTURE)
		{
			...
			glBindTexture(GL_TEXTURE_2D,material->tex_map);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		}
